...
import 'package:background_locator_example/service.dart';
...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> initPlatformState() async {
    ...
    service.getUser('IGUfkMA2A0AUx4M4HqKN').listen((event) {
      setState(() {
        docId = event.docId;
      });
    });
  ...
  Widget build(BuildContext context) {
    final docName = Text("firestore: $docId");
    ...
    if (isRunning != null) {
      if (isRunning) {
        if (lastTimeLocation == null || lastLocation == null) {
          ...
          lastRunTxt = LocationServiceRepository.formatDateLog(lastTimeLocation) + "-" + LocationServiceRepository.formatLog(lastLocation);
        }
      }
    }
    final lastRun = Text(
        ...
        body: Container(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: <Widget>[start, stop,docName, clear, status, lastRun, log],
            ),
          ),
        ),
      ),
